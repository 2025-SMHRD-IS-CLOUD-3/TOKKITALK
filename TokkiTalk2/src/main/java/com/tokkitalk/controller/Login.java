...
public class Login extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// 3. Member 객체 생성
		MevenMember member = new MevenMember(null, user_id, user_pw, null, null, null);
		// 4. DAO 객체 생성
		...
		if ("1".equals(ajax)) {
			...
			try (PrintWriter out = response.getWriter()) {
				if (result != null) {
					...
					out.write("{\"success\":true,\"userId\":\"" + result.getUser_id() + "\",\"userName\":\""
							+ (result.getUser_name() != null ? result.getUser_name() : result.getUser_id()) + "\"}");
				} else {
